...
## Overview
`limb16-imul-import` is a variation of the 2-stage pipelined 16-bit limb multiplication algorithm (`limb16-pipe-imul-all`) where every multiplication is dispatched through a user-defined helper function `mul(a, b)` imported from `src/u32/mul` (`Math.imul(a, b) >>> 0`).
