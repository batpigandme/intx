...
const k = 13 | 0;
const d = 17 | 0;
const arg = operand === "literal" ? (kernel === "rotl" ? "13" : "17") : kernel === "rotl" ? "k" : "d";
const loop =
	kernel === "rotl"
		? `acc = rotl(acc, ${arg});`
...
const run = new Function(
	...
	"d",
	"buf",
	`return function kernel(iters, tic, toc) { ${setup} tic(); for (let i = 0; i < iters; i++) { ${loop} } toc(); return acc; };`,
...
let sink = 0;
const N = 1e7;
if (pattern === "warm") for (let j = 0; j < 2000; j++) sink ^= run(1e4, tic, toc);
