...
- **CPU Affinity & Core Pinning**: Always pin hardware cycle benchmark runs to an isolated physical core (Core 2, via `taskset -c 2` or `sched_setaffinity`) to eliminate Linux scheduler migration penalties, L1/L2 cache evictions, and frequency scaling jitter.
